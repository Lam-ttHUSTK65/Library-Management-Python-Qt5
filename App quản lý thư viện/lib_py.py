...
uiii, _ = loadUiType('library2.ui')


class LoginApp(QMainWindow, uii):
    def __init__(self):
    ...
    def login(self):
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            QMessageBox.warning(self, "Invalid Email", "Invalid email format. Please enter a valid email address.")
            return

        cursor.execute("select * from user where email = %s", (email,))
        result = cursor.fetchone()
        if not result:
            QMessageBox.warning(self, "Wrong Email", "Wrong Email! Please try again!")
        elif result[3] != pw:
            QMessageBox.warning(self, "Wrong Password", "Password is not correct! Please try again!")
        else:
            is_ad = result[4]
            QMessageBox.information(self, "Login output", "Login successful")
            if is_ad == 1:
                QMessageBox.information(self, "Welcome", "Hi admin")
                self.window2 = MainApp(email)
            elif is_ad == 0:
                QMessageBox.information(self, "Welcome", "Hi user")
                self.window2 = MainAppUser(email)
            self.window2.show()
            self.close()
...
class MainAppUser(QMainWindow, ui):
    ...
    def button_trigger(self):
        ...
        self.pushButton_55.clicked.connect(self.Log_Out)
    ...
    def Change_password(self):
        ...
        current_password_db = self.cur.fetchone()[0]
...
class MainApp(QMainWindow, uiii):
    ...
    def Export_today_loan(self):
        ...
        result = cur.fetchall()
        df = pd.DataFrame(result, columns=["Loan ID", "User Name", "Book Title", "Copy ID", "Catagory", "Publisher",
                                           "Loan date", "Due date", "Status", "Fine", "Reason"])
    ...
    def Add_New_Book(self):
        ...
        if not all([book_title, book_description, book_isbn, publication_year, category_name, publisher_name]):
            ...
            QMessageBox.warning(self, "Please fill in all the required information.")
    ...
    def Delete_Book(self):
        ...
        if book:
            ...
            if loans:
                QtWidgets.QMessageBox.information(self, "Cannot delete book",
                                                  "This book cannot be deleted because there are unreturned copies of it.")
            else:
            ...
            QtWidgets.QMessageBox.information(self, "Book not found",
                                              "No book with that ISBN was found in the database.")
    ...
    def Search_User(self):
        ...
        if user:
            ...
            if user[4] == 1:
                QtWidgets.QMessageBox.information(self, "User not found",
    ...
    def Edit_User(self):
        ...
        if user:
            ...
            # Check if the password again matches the password
                ...
                db.commit()
                QtWidgets.QMessageBox.information(self, "User updated",
    ...
    def Search_Author_Book(self):
        ...
        if author:
            ...
            cursor.execute("SELECT b.Title FROM book b INNER JOIN writing w ON b.ID = w.Book_ID WHERE w.Author_ID = %s",
                           (author[0],))
            ...
            if books:
                ...
                QtWidgets.QMessageBox.information(self, "No books found",
                                                  "No books written by this author were found in the database.")
        else:
            QtWidgets.QMessageBox.information(self, "Author not found",
                                              "No author with that name was found in the database.")
